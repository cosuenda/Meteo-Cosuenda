...
function actualizarLluvia(rain){
    ...
    if(!datos) datos={mes, año,totalMes:rain,totalAño:rain};
        ...
        if(datos.mes!==mes) datos.totalMes=rain, datos.mes=mes;
        else if(rain>datos.totalMes) datos.totalMes=rain;
        if(datos.año!==año) datos.totalAño=rain, datos.año=año;
        else if(rain>datos.totalAño) datos.totalAño=rain;
    }
...
function actualizarFlecha(grados, vel){
    ...
    if(!flecha) return;
    let diff=grados-angAnterior; if(diff>180) diff-=360; if(diff<-180) diff+=360;
    angAnterior+=diff;
